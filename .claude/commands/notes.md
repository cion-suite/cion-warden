...
- **Theme** — короткое русское слово (≤ 15 символов). Предпочтительно: `фиксы · скрипты · рефакторинг · документация · перформанс · UI · установка · обновление · vault · библиотеки · пресеты · биндинги · источники · безопасность`.
...
[USERS]
- Исправлена загрузка скриптов из удалённого vault'а.
- Исправлено отображение пресетов после переключения источника.

[GIT]
- Исправлена обработка таймаута при синхронизации vault.
- Исправлена кодировка имён скриптов при чтении manifest'а.
- Добавлен retry при rate-limit GitHub API в источниках.
```
