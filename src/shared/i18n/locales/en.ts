export const en = {
    common: {
        hello: 'Hello',
        cancel: 'Cancel',
        confirm: 'Confirm',
        close: 'Close',
        loading: 'Loading…',
        retry: 'Retry',
        error: 'Error',
        ok: 'OK',
        yes: 'Yes',
        no: 'No',
        nav: {
            home: 'Home',
            settings: 'Settings',
            scripts: 'Scripts',
            getScripts: 'Get Scripts',
            libraries: 'Libraries',
            binds: 'Binds',
            unknown: 'Unknown',
            toggleSidebar: 'Toggle sidebar',
            section: {
                main: 'Main',
                local: 'Local',
                sources: 'Sources',
                misc: 'Misc',
            },
        },
        theme: {
            toggle: 'Toggle theme',
            light: 'Light',
            dark: 'Dark',
        },
        locale: { toggle: 'Toggle language' },
        home: {
            tabs: {
                overview: 'Overview',
                diagnostics: 'Diagnostics',
                activity: 'Activity',
            },
            status: {
                ok: 'OK',
                warn: 'Pending',
                off: 'Off',
            },
            overview: {
                runtime: 'Runtime',
                runtimeWaiting: 'Waiting…',
                theme: 'Theme',
                locale: 'Language',
                updater: 'Version',
            },
            runtime: {
                title: 'Runtime',
                description: 'app:ready event from main process',
                waiting: 'Waiting for app:ready…',
                ready: 'Ready at',
            },
            theme: {
                title: 'Theme',
                description: 'next-themes provider',
                current: 'Current',
                resolved: 'Resolved',
            },
            i18n: {
                title: 'Localization',
                description: 'i18next + react-i18next',
                current: 'Current locale',
            },
            ui: {
                title: 'UI primitives',
                description: 'shadcn + sonner + radix smoke test',
                toast: 'Toast',
                tooltip: 'Tooltip',
            },
            updater: {
                title: 'Updater bridge',
                description: 'window.app.updater',
                available: 'Bridge available',
                missing: 'Bridge missing — preload not wired',
            },
            activity: {
                empty: 'No events yet. State changes will appear here.',
                clear: 'Clear',
                events: {
                    ready: 'Main process ready',
                    themeChanged: 'Theme changed',
                    localeChanged: 'Language changed',
                },
            },
        },
        scripts: {
            title: 'Scripts',
            stopAll: 'Stop All',
            search: 'Search scripts…',
            refresh: 'Refresh',
            run: 'Run',
            stop: 'Stop',
            openInExplorer: 'Open in Explorer',
            delete: 'Delete',
            deleteConfirm: 'Move "{{name}}" to trash?',
            modified: 'Modified',
            empty: 'No scripts found. Add script files to your vault.',
            configDialog: {
                title: 'Configure',
                hotkeys: 'Hotkeys',
                values: 'Values',
                save: 'Save',
                pressKey: 'press key…',
            },
        },
        getScripts: {
            title: 'Get Scripts',
            empty: 'No scripts available. Add a source to get started.',
            get: 'Get',
            update: 'Update',
            upToDate: 'Up to date',
            updateAvailable: 'Update',
            downloaded: '"{{name}}" downloaded',
        },
        libs: {
            title: 'Libraries',
            empty: 'No libraries available. Add a source to get started.',
            search: 'Search libraries…',
            download: 'Download',
            update: 'Update',
            latest: 'Latest',
            downloadAll: 'Download All',
            openLocal: 'Open local folder',
            openWeb: 'Open in browser',
            deleteLocal: 'Delete local',
            deleteConfirm: 'Delete local copy of "{{name}}"?',
            downloaded: '"{{name}}" downloaded',
            deleted: '"{{name}}" deleted',
            bulkDone: '{{ok}} libraries downloaded',
            bulkDoneWithFails: '{{ok}} downloaded, {{failed}} failed',
        },
        binds: {
            title: 'Binds',
            empty: 'No presets available. Add a source to get started.',
            download: 'Download',
            update: 'Update',
            downloadAll: 'Download All',
            openLocal: 'Open local file',
            deleteLocal: 'Delete local',
            deleteConfirm: 'Delete local copy of "{{name}}"?',
            downloaded: '"{{name}}" downloaded',
            deleted: '"{{name}}" deleted',
            bulkDone: '{{ok}} presets downloaded',
            bulkDoneWithFails: '{{ok}} downloaded, {{failed}} failed',
            back: 'Back',
            reset: 'Reset',
            save: 'Save',
            saved: 'Saved',
            schemaEmpty: 'Preset has no fields.',
        },
        sources: {
            title: 'Sources',
            add: 'Add Source',
            addNew: 'Add New Source',
            edit: 'Edit Source',
            save: 'Save',
            name: 'Name',
            namePlaceholder: 'My Scripts',
            type: 'Type',
            external: 'External',
            url: 'Repository URL',
            branch: 'Branch',
            private: 'Private',
            scriptsUrl: 'Scripts URL',
            libsUrl: 'Libraries URL',
            cfgUrl: 'Config URL',
            sync: 'Sync',
            syncing: 'Syncing…',
            lastSynced: 'Last synced:',
            neverSynced: 'Never synced',
            scriptsCount: 'Scripts ({{count}})',
            downloadAll: 'Download All',
            selectSource: 'Select a source to view its settings',
            noScripts: 'No scripts found. Click Sync to fetch from remote.',
            newSource: 'New source…',
            syncDone: 'Synced successfully',
            repoNotFound: 'Repository not found. May have been deleted or made private — verify URL and visibility',
            token: 'Personal access token',
            tokenPlaceholder: 'github_pat_...',
            tokenChange: 'Replace token',
            tokenInvalid: 'Invalid token',
            tokenNoAccess: 'Token does not grant access to this repository',
            tokenSaved: 'Token saved',
            tokenSaveError: 'Failed to save token',
        },
        settings: {
            groups: {
                appearance: 'Appearance',
                github: 'GitHub API',
                updates: 'Updates',
            },
            tabs: {
                appearance: 'Appearance',
                github: 'GitHub',
                updates: 'Updates',
            },
            appearance: {
                title: 'Appearance',
                description: 'Theme and interface language.',
            },
            updates: {
                title: 'Updates',
                description: 'Auto-check runs hourly and at startup.',
            },
            theme: {
                label: 'Theme',
                description: 'How the UI renders.',
            },
            locale: {
                label: 'Language',
                description: 'Interface language.',
            },
            github: {
                title: 'Rate limit',
                description: 'GitHub REST API quota for each saved token. Refreshes automatically after every sync.',
                refresh: 'Refresh',
                anonymous: 'Unauthenticated (60/hr)',
                remainingResets: '{{remaining}} remaining · resets {{resetIn}}',
                resetNow: 'now',
                resetInSeconds: 'in {{seconds}}s',
                resetInMinutes: 'in {{minutes}}m {{seconds}}s',
                resetInHours: 'in {{hours}}h {{minutes}}m',
            },
            updater: {
                label: 'Updates',
                description: 'Checks run hourly and at startup.',
                unavailable: 'Updater bridge unavailable in dev preview.',
                checking: 'Checking…',
                checkNow: 'Check for updates',
                rateLimited: 'Next check in {{seconds}}s',
                notAvailable: 'No updates — you are on the latest version',
                available: 'Update available v{{version}}',
                downloaded: 'v{{version}} ready to install',
                install: 'Restart',
                currentVersion: 'Current version',
                statusLabel: 'Status',
                statusIdle: 'Not checked yet',
                statusUpToDate: 'You are on the latest version',
                statusAvailable: 'Update v{{version}} found, downloading…',
                statusDownloaded: 'v{{version}} downloaded and ready to install',
                installNow: 'Install and restart',
            },
        },
    },
} as const;
